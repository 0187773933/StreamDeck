...
func main() {
	CWD , _ = os.Getwd()
	CWD = fmt.Sprintf( "%s/test3" , CWD ) // temp addon because we are for some reason still in sub-folder

	ui := NewStreamDeckUI( "./test3/config.yaml" )
	ui.AddDevice()
